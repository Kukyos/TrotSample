...
Everything we've locked in so far. This is a plan, not repo files. The actual
`AGENTS.md`, schema, and service contract get written **on the day**, once the
real problem statement is in hand.
...
| file | purpose |
|---|---|
| `AGENTS.md` | what Codex reads — stack, ownership lanes, hard rules, conventions |
| `CLAUDE.md` | identical copy — Claude Code reads a different filename |
